...
def draw_side_by_side_evaluation_images(eval_dict,
                                       category_index,
                                       max_images=1,
                                       max_boxes_to_draw=20,
                                       min_score_thresh=0.2):
    ...
    for i in range(max_images):
        image = tf.cast(eval_dict['images'][i:i+1], tf.uint8)
        detection_boxes = eval_dict['detection_boxes'][i]
        detection_classes = (
                tf.to_int64(eval_dict['detection_classes'][i]) +
                    label_id_offset)
        detection_scores = eval_dict['detection_scores'][i]

        num_detections = tf.to_int32(eval_dict['num_detections'][i])
        ...
        groundtruth_classes = (
            tf.argmax(eval_dict['groundtruth_classes'][i], axis=1) + label_id_offset)
        single_eval_dict = {
            'images': image,
            'groundtruth_boxes': eval_dict['groundtruth_boxes'][i],
            'groundtruth_classes': groundtruth_classes,
            'detection_boxes': detection_boxes,
            'detection_classes': detection_classes,
            'detection_scores': detection_scores,
        }
        draw_image = draw_side_by_side_evaluation_image(single_eval_dict,
               category_index,
               max_boxes_to_draw=max_boxes_to_draw,
               min_score_thresh=min_score_thresh)
...
def draw_side_by_side_evaluation_image(eval_dict,
  ...
  images_with_detections = draw_bounding_boxes_on_image_tensors(
      eval_dict['images'],
      tf.expand_dims(eval_dict['detection_boxes'], axis=0),
      tf.expand_dims(eval_dict['detection_classes'], axis=0),
      tf.expand_dims(eval_dict['detection_scores'], axis=0),
      category_index,
      instance_masks=None,
      keypoints=None,
      max_boxes_to_draw=max_boxes_to_draw,
  ...
  images_with_groundtruth = draw_bounding_boxes_on_image_tensors(
      eval_dict['images'],
      tf.expand_dims(eval_dict['groundtruth_boxes'], axis=0),
      tf.expand_dims(eval_dict['groundtruth_classes'], axis=0),
      tf.expand_dims(
          tf.ones_like(
              eval_dict['groundtruth_classes'],
              dtype=tf.float32),
          axis=0),
      category_index,
      instance_masks=None,
