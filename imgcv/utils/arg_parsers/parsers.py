...
class ImageDebugParser(argparse.ArgumentParser):
  """Default parser for specification image specific behavior.
  ...
  def __init__(self, add_help=False, image_path=True):
    super(ImageDebugParser, self).__init__(add_help=add_help)
    if image_path:
      self.add_argument(
          "--image_path", "-ip",
          default=None,
          help="[default: %(default)s] image path",
          metavar="<IP>"
      )
