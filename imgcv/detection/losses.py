...
class WeightedIOULocalizationLoss(Loss):
  ...
  def _compute_loss(self, prediction_tensor, target_tensor, weights):
    ...
    per_anchor_iou_loss = 1.0 - pp.box.matched_iou(predicted_boxes,
                                                         target_boxes)
...
class HardExampleMiner(object):
  ...
  def __call__(self,
    ...
    Args:
      ...
      detection_boxes: a list of decoded BoxList representing location
        predictions for each image.
      matches_list: an optional list of matcher.Match objects encoding the match
        between anchors and groundtruth boxes for each image of the batch,
        with rows of the Match objects corresponding to groundtruth boxes
        and columns corresponding to anchors.  Match objects in matches_list are
    ...
    Returns:
      ...
      mined_cls_loss: a float scalar with sum of classification losses from
        selected hard examples.
    Raises:
      ValueError: if location_losses, cls_losses and detection_boxes do
        not have compatible shapes (i.e., they must correspond to the same
        number of images).
      ValueError: if matches_list is specified but its length does not match
        len(detection_boxes).
    """
    ...
    num_images = len(detection_boxes)
    if not len(location_losses) == len(detection_boxes) == len(cls_losses):
      raise ValueError('location_losses, cls_losses and detection_boxes '
  ...
  def _subsample_selection_to_desired_neg_pos_ratio(self,
    ...
    Returns:
      ...
      num_negatives: An integer tensor representing the number of negative
        examples in selected set of indices.
    """
    positives_indicator = tf.gather(tf.greater_equal(matches, 0), indices)
    negatives_indicator = tf.gather(tf.equal(matches, -1), indices)
