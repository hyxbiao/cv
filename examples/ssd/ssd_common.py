...
class SSDDataSet(DetectionDataSet):
    def __init__(self, flags, num_classes):
        super(SSDDataSet, self).__init__(flags)
...
class SSDModel(ssd.SSDVGGModel):
    pass


class SSDEstimator(detection.Estimator):
    def __init__(self, flags, num_classes, category_index):
        super(SSDEstimator, self).__init__(flags, category_index)
...
class SSDRunner(detection.Runner):
    def __init__(self, flags, estimator, dataset):
        shape = [SSD_VGG_SIZE, SSD_VGG_SIZE, NUM_CHANNELS]
        super(SSDRunner, self).__init__(flags, estimator, dataset, shape)
...
class SSDArgParser(detection.ArgParser):
    def __init__(self):
        super(SSDArgParser, self).__init__()
        self.add_argument(
